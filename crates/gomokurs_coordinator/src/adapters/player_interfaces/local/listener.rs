...
impl PlayerListener for Local {
    async fn listen(
        ...
        loop {
            ...
            match convert_input_to_player_action(line) {
                Ok(action) => {
...
fn convert_input_to_player_action(input: String) -> Result<PlayerAction, InputConvertionError> {
    lazy_static! {
    ...
    if RE_OK.is_match(&input) {
        ...
        let infos = parse_player_informations(&input);
